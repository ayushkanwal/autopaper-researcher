...
PRESETS: Dict[str, QueryPreset] = {
    "livestock_decision_support_v2": QueryPreset(
        ...
        queries=[
            'all:"multivariate time series" AND (all:forecasting OR all:prediction OR all:reasoning OR all:analysis) AND (all:llm OR all:"foundation model" OR all:agent OR all:"tool-augmented" OR all:retrieval OR all:rag OR all:orchestration) AND (all:"decision support" OR all:"digital twin" OR all:mechanistic OR all:biophysical OR all:recommendation OR all:scenario)',
            'all:"time series" AND (all:"decision support" OR all:"scenario planning" OR all:recommendation OR all:uncertainty OR all:calibration OR all:"anomaly detection") AND (all:livestock OR all:pasture OR all:forage OR all:paddock OR all:grazing OR all:"body weight" OR all:adg)',
            ...
            '((retrieval OR rag OR "case-based") AND ("time series" OR forecasting)) AND (reasoning OR agent OR tool OR orchestration) AND ("decision support" OR recommendation OR scenario OR livestock OR pasture)',
